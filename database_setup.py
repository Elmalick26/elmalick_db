            ...
            def _ensure_column(table_name, column_name, column_sql):
                if not _column_exists(table_name, column_name):
                    try:
                        ...
                        logger.error(f"Migration failed for {table_name}.{column_name}: {e}")
...
def log_audit(conn, actor: str, action: str, target: str) -> None:
    ...
    يجب استدعاؤها داخل نفس كائن الاتصال المفتوح لضمان الـ atomicity.

    المعاملات:
        conn   : كائن اتصال psycopg2 مفتوح
        actor  : اسم المستخدم المنفِّذ للعملية
        action : رمز العملية مثل 'LOGIN', 'ADD_STUDENT', 'PAYMENT'
        target : وصف الكيان المستهدف مثل اسم الطالب أو رقم الوصل

    مثال:
        with db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO Students ...", (...))
            log_audit(conn, "admin", "ADD_STUDENT", "Ahmed Ba")
    """
    try:
        cursor = conn.cursor()
        cursor.execute(
