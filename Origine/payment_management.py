...
class StudentDuesWindow(QMainWindow):
    ...
    def _ensure_student_dues_schema(self):
        try:
            ...
            with db.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("PRAGMA table_info(StudentDues)")
    ...
    def apply_discount(self):
        ...
        try:
            ...
            with db.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT is_paid FROM StudentDues WHERE id=?", (due_id,))
                res = cursor.fetchone()
                if res and res[0]:
                ...
                cursor.execute("""
                    UPDATE StudentDues 
                    SET discount_amount=?, net_amount=? 
                    WHERE id=?
    ...
    def delete_due(self, due_id):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                with db.get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute("DELETE FROM StudentDues WHERE id=?", (due_id,))
                    conn.commit()
                self.load_student_dues()
            except Exception as e:
                QMessageBox.critical(self, "Erreur", str(e))
    ...
    def generate_auto_dues(self):
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                cursor.execute("SELECT month_index, month_name, amount FROM MonthlyFeeSchedule WHERE class_id=?", (cid,))
                ...
                for (sid,) in students:
                    if reg_amt > 0:
                        cursor.execute("SELECT COUNT(*) FROM StudentDues WHERE student_id=? AND year_id=? AND fee_type='Registration'", (sid, active_year))
                        if cursor.fetchone()[0] == 0:
                            cursor.execute("""
                                INSERT INTO StudentDues (student_id, year_id, fee_type, fee_description, original_amount, net_amount, due_date)
                                VALUES (?, ?, 'Registration', 'Frais d''inscription', ?, ?, ?)
                            """, (sid, active_year, reg_amt, reg_amt, today_str))
                            generated_count += 1
                            
                    for m_idx, m_name, m_amt in monthly_fees:
                        cursor.execute("SELECT COUNT(*) FROM StudentDues WHERE student_id=? AND year_id=? AND fee_type=?", (sid, active_year, f'Month_{m_idx}'))
                        if cursor.fetchone()[0] == 0:
            ...
            with db.get_connection() as conn:
                ...
                cursor.execute("""
                    SELECT
                        ...
                        COALESCE((SELECT year_label FROM AcademicYears WHERE id = ?), '-')
                    FROM Students S
