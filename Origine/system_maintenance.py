...
class SystemMaintenanceWindow(QMainWindow):
    def __init__(self):
        ...
        # Database and Backup paths setup safely
        ...
        try:
            ...
            print(f"Warning: Could not create backup directory at {self.backup_dir}: {e}")
    ...
    def create_backup(self):
        if not os.path.exists(self.db_name):
            QMessageBox.warning(self, "Erreur", "Fichier de base de données introuvable.")
            return
    ...
    def import_backup(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "Importer une base de données", "", "Database Files (*.db *.sqlite *.bak)")
        if file_path:
            try:
    ...
    def restore_backup(self, filename):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                safety_backup_path = os.path.join(self.backup_dir, f"safety_before_restore_{timestamp}.db")
                if os.path.exists(self.db_name):
                    shutil.copy2(self.db_name, safety_backup_path)
                
                # 2. Restore selected backup
                source = os.path.join(self.backup_dir, filename)
                shutil.copy2(source, self.db_name)
                
                QMessageBox.information(self, "Succès", "Restauration terminée. L'application va se fermer pour appliquer les changements.")
