...
class ExpensesWindow(QMainWindow):
    ...
    def fetch_expense_report_data(self, report_key, from_date, to_date):
        ...
        # إصلاح خطأ استعلام التواريخ (لتشمل المدفوعات التي حدثت في أوقات متأخرة من اليوم الأخير)
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                if report_key == "monthly_category":
                    ...
                    cursor.execute(
                        """
                        SELECT substr(expense_date, 1, 7) AS period, COALESCE(SUM(amount), 0)
                        FROM Expenses
                        WHERE expense_date BETWEEN ? AND ?
                        GROUP BY period
                        """,
                    ...
                    # The precision logic to match Datetime formatting in Payments table
                    cursor.execute(
                        """
                        SELECT substr(transaction_date, 1, 7) AS period, COALESCE(SUM(amount_paid), 0)
                        FROM Payments
                    ...
                    for expense_date, category, description, paid_to, amount in cursor.fetchall():
                        ...
                        total_expenses += amount_value
                        # Only take the date part for clean UI if datetime is saved
                        display_date = expense_date.split(" ")[0] if expense_date else ""
                        rows.append([
                            display_date,
                            category or "",
                            description or "",
