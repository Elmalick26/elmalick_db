...
class GradeCalculator:
    ...
    def _get_attendance_count(self, cursor, student_id, status, year_id, period_id=None):
        cursor.execute("PRAGMA table_info(StudentAttendance)")
        ...
        if period_id and has_period_col:
            cursor.execute(
                "SELECT COUNT(*) FROM StudentAttendance WHERE student_id=? AND status=? AND period_id=?",
    ...
    def _get_discipline_data(self, cursor, student_id, year_id, is_primary, period_id=None):
        cursor.execute("PRAGMA table_info(StudentDiscipline)")
        discipline_cols = {row[1] for row in cursor.fetchall()}
    ...
    def get_student_bulletin_data(self, student_id, class_id, period_id):
        ...
        with db.get_connection() as conn:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM StudentClassNumbers WHERE class_id=? AND year_id=?",
                (class_id, year_id)
            )
...
class BulletinGenerationWindow(QMainWindow):
    ...
    def load_filters(self):
        try:
            ...
            with db.get_connection() as conn:
                ...
                if active_year != -1:
                    cursor.execute(
                        "SELECT id, period_name_fr, period_name_ar FROM AcademicPeriods WHERE year_id=? ORDER BY sort_order",
                        (active_year,)
                    )
                else:
    ...
    def load_students_indiv(self):
        ...
        try:
            ...
            print(f"Error loading students indiv: {e}")
    ...
    def get_real_period_id(self, class_id, period_name):
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                if active_year != -1:
                    cursor.execute(
                        "SELECT id FROM AcademicPeriods WHERE period_name_fr=? AND cycle_id=? AND year_id=? ORDER BY sort_order LIMIT 1",
                ...
                cursor.execute(
                    "SELECT id FROM AcademicPeriods WHERE period_name_fr=? AND cycle_id=? ORDER BY id DESC LIMIT 1",
    ...
    def generate_bulletins(self, class_id, period_name, specific_ids=None):
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                cursor.execute("SELECT year_label FROM AcademicYears WHERE id=?", (year_id,))
                yr = cursor.fetchone()
                if yr:
                    year_label = yr[0]
                else:
