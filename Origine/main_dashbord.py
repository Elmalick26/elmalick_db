...
class MainWindow(QMainWindow):
    ...
    def toggle_theme(self):
     ...
     # التعديل هنا: تحديث دالة الأيقونة
    ...
    def load_dashboard_data(self):
        try:
            ...
            with db.get_connection() as conn:
                ...
                # Students
                cursor.execute("SELECT COUNT(*) FROM Students S JOIN StudentClassNumbers SCN ON S.id=SCN.student_id WHERE S.status='Active' AND SCN.year_id=?", (active_year,))
                ...
                cursor.execute("SELECT SUM(amount_paid) FROM Payments WHERE year_id=?", (active_year,))
