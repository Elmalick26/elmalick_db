        ...
        try:
            ...
            if self.current_photo_path:
                ...
                with db.get_connection() as conn:
                    ...
                    if res and res[0]: saved_photo_path = res[0]
    ...
    def delete_staff_from_table(self, pid):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                db = DatabaseManager()
                with db.get_connection() as conn:
                    # Soft delete is used to maintain history of payments/timetables
                    conn.execute("UPDATE Staff SET status='Archived' WHERE id=?", (pid,))
                    conn.commit()
                self.load_staff_list()
                self.clear_form()
            except Exception as e:
