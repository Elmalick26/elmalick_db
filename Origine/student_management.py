...
class ModernStudentManagement(QMainWindow):
    ...
    def setup_list_tab(self):
        ...
        self.table_students.setHorizontalHeaderLabels([
            "ID", "Prénom & Nom (FR)", "الاسم واللقب (AR)", "Sexe", "Classe", "N° Classe", "Tuteur", "Tél", "Date", "Actions"
        ])
    ...
    def get_next_class_number(self, class_id, year_id):
        ...
        with db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT MAX(class_number) FROM StudentClassNumbers WHERE class_id = ? AND year_id = ?", (class_id, year_id))
            row = cursor.fetchone()
            return (row[0] or 0) + 1
    ...
    def get_student_class_number(self, student_id, year_id):
        ...
        with db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT class_id, class_number FROM StudentClassNumbers WHERE student_id = ? AND year_id = ?", (student_id, year_id))
            row = cursor.fetchone()
            return row
    ...
    def assign_class_number(self, student_id, class_id):
        ...
        with db.get_connection() as conn:
            cursor = conn.cursor()
            if existing:
                cursor.execute("UPDATE StudentClassNumbers SET class_id = ?, class_number = ? WHERE student_id = ? AND year_id = ?", (class_id, new_number, student_id, year_id))
            else:
                cursor.execute("INSERT INTO StudentClassNumbers (student_id, class_id, year_id, class_number) VALUES (?, ?, ?, ?)", (student_id, class_id, year_id, new_number))
            conn.commit()
            return new_number
    ...
    def add_student(self):
        ...
        try:
            ...
            with db.get_connection() as conn:
                cursor = conn.cursor()
                # إزالة class_id من جملة الـ INSERT هنا لتتوافق مع القاعدة الجديدة
                cursor.execute("""
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                params = [
                    ...
                    self.txt_birth_place.text(), 
                    self.gender_to_db_value(),
                    self.txt_address.text(), self.txt_parent_name.text(), 
                    self.txt_parent_phone.text(), self.txt_parent_email.text(), 
                    self.txt_parent_addr.text(),
                    self.date_registration.date().toString("yyyy-MM-dd"), 
    ...
    def load_student_for_edit(self, student_id):
        try:
            ...
            with db.get_connection() as conn:
                cursor = conn.cursor()
                # جلب بيانات الطالب مع class_id من جدول التسجيل بناء على السنة النشطة
                cursor.execute("""
                    SELECT S.first_name_fr, S.last_name_fr, S.first_name_ar, S.last_name_ar,
                           ...
                           SCN.class_id
                    FROM Students S
                    LEFT JOIN StudentClassNumbers SCN ON S.id = SCN.student_id AND SCN.year_id = ?
                    WHERE S.id = ?
                """, (active_year, student_id))
            ...
            if data:
                ...
                if data[4]: self.date_birth.setDate(QDate.fromString(data[4], "yyyy-MM-dd"))
                ...
                if data[12]: self.date_registration.setDate(QDate.fromString(data[12], "yyyy-MM-dd"))
    ...
    def populate_table(self, table):
        ...
        with db.get_connection() as conn:
            cursor = conn.cursor()
            # تعديل الـ JOIN ليتم مع SCN بدلاً من S
            query = """
            ...
            if class_id:
                query += " AND SCN.class_id = ?" # عدلنا هنا أيضا
            ...
            if search:
                query += " AND (S.last_name_fr LIKE ? OR S.first_name_fr LIKE ? OR S.last_name_ar LIKE ? OR S.first_name_ar LIKE ?)"
                s_param = f"%{search}%"
    ...
    def print_student_list(self):
        ...
        headers = [
            ...
            "Naissance", "Lieu", "Sexe", "Adresse", "Classe", "N° Cls",
