...
class FeesSetupWindow(QMainWindow):
    ...
    def load_reg_table(self):
        ...
        try:
            ...
            print(f"Error loading registration table: {e}")
    ...
    def load_monthly_schedule(self):
        ...
        try:
            ...
            print(f"Error loading monthly schedule: {e}")
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                if report_key == "comparison":
                    ...
                    # تم إصلاح الخطأ: البحث في جدول التسجيل عن طريق SCN.student_id وليس S.class_id المفقود
                    cursor.execute("""
                        SELECT C.class_name_fr,
                               COALESCE((
                                   ...
                                   WHERE SCN.class_id = C.id AND SCN.year_id = ? AND S.status = 'Active'
