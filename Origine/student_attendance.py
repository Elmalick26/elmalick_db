...
class StudentAttendanceWindow(QMainWindow):
    ...
    def load_students_for_entry(self):
        ...
        try:
            ...
            with db.get_connection() as conn:
                cursor = conn.cursor()
                # تم التعديل: قراءة الطلاب من StudentClassNumbers
                query = """
                    SELECT S.id, S.first_name_fr || ' ' || S.last_name_fr, 
                        A.status, A.justifie, A.reason, A.notes
                    FROM Students S
                    JOIN StudentClassNumbers SCN ON S.id = SCN.student_id AND SCN.year_id = ?
                    LEFT JOIN StudentAttendance A ON S.id = A.student_id AND A.date = ? AND A.year_id = ?
                    WHERE SCN.class_id = ? AND S.status = 'Active'
                    ORDER BY S.first_name_fr
                """
                params = [active_year, date_sel, active_year, class_id]
                if period_id:
                    query = query.replace("LEFT JOIN StudentAttendance A ON S.id = A.student_id AND A.date = ? AND A.year_id = ?", "LEFT JOIN StudentAttendance A ON S.id = A.student_id AND A.date = ? AND A.year_id = ? AND A.period_id = ?")
            ...
            with db.get_connection() as conn:
                ...
                for row in range(self.table_entry.rowCount()):
                    ...
                    if resolved_period_id:
                        cursor.execute("SELECT id FROM StudentAttendance WHERE student_id = ? AND date = ? AND year_id = ? AND COALESCE(period_id, 0) = ?", (sid, date_sel, active_year, resolved_period_id))
                    else:
                        cursor.execute("SELECT id FROM StudentAttendance WHERE student_id = ? AND date = ? AND year_id = ? AND (period_id IS NULL OR period_id = 0)", (sid, date_sel, active_year))
                    exists = cursor.fetchone()

                    if exists:
                        cursor.execute("""
                            UPDATE StudentAttendance 
                            SET status=?, justifie=?, reason=?, notes=?, year_id=?, period_id=?
                            WHERE id=?
    ...
    def load_students_for_report_combo(self):
        ...
        try:
            ...
            print(f"Error loading students for report: {e}")
    ...
    def fetch_report_data(self):
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                if pid:
                    query += " AND COALESCE(A.period_id, 0) = ?"
                    params.append(pid)
                
                if cid:
                    query += " AND SCN.class_id = ?"
                    params.append(cid)
                if sid:
    ...
    def generate_pdf_report(self, report_type):
        ...
        class_slug = self.sanitize_filename(self.combo_class_report.currentText() if hasattr(self, 'combo_class_report') else "Toutes_Classes")
        ...
        if report_type == "daily":
            ...
            default_name = f"Rapport_Individuel_{student_slug}_{date_from_str}_{date_to_str}_{timestamp}.pdf" if student_slug else f"Rapport_Individuel_{date_from_str}_{date_to_str}_{timestamp}.pdf"
