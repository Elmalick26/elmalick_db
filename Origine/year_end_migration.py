    ...
    def run(self):
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                period_ids = []
                try:
                    # ===== تم الإصلاح: اسم الجدول الصحيح AcademicPeriods =====
                    cursor.execute("SELECT id FROM AcademicPeriods WHERE year_id=?", (self.current_year_id,))
                    period_ids = [row[0] for row in cursor.fetchall()]
                except Exception as e:
                    print(f"Error fetching periods: {e}")
                    period_ids = []

                # ===== تم الإصلاح: جلب الطلاب من SCN للسنة الحالية =====
                if self.filter_class_id:
                    cursor.execute("""
                        ...
                        WHERE S.status='Active' AND SCN.class_id=? AND SCN.year_id=?
                ...
                for i, (std_id, fname, lname, class_id) in enumerate(students):
                        ...
                        for pid in period_ids:
                            ...
                            for sub_id, coef in subjects:
                                cursor.execute("""
                                    ...
                                    WHERE G.student_id=? AND G.subject_id=? AND A.period_id=? AND G.year_id=?
                    ...
                    if period_avgs:
                        ...
                        cursor.execute("SELECT AVG(score) FROM Grades WHERE student_id=? AND year_id=?", (std_id, self.current_year_id))
                        res = cursor.fetchone()
                    ...
                    if cname_res:
                        ...
                        if "elem" in cname or "prim" in cname or "ibtida" in cname:
                            threshold = 5.0
                    ...
                    if decision == "Admis":
                        ...
                        if not found_next:
                            next_class_id = None
                            next_class_name = "Fin de Cycle / Diplômé (تخرج)"
        ...
        with DatabaseManager() as db:
            ...
            try:
                ...
                for idx in range(self.table_preview.rowCount()):
                    ...
                    elif not new_class_id and decision == "Admis":
                        cursor.execute("UPDATE Students SET status='Diplômé' WHERE id=?", (std_id,))
                    elif new_class_id:
                        # 1. حساب رقم الطالب في الفصل الجديد
                        cursor.execute("SELECT MAX(class_number) FROM StudentClassNumbers WHERE class_id=? AND year_id=?", (new_class_id, target_year_id))
                        max_num_res = cursor.fetchone()
                        next_num = (max_num_res[0] or 0) + 1

                        # 2. إنشاء السجل الأكاديمي للعام الجديد (أو تحديثه إن وجد بالخطأ)
                        cursor.execute("SELECT id FROM StudentClassNumbers WHERE student_id=? AND year_id=?", (std_id, target_year_id))
                        exists = cursor.fetchone()
                        
                        if exists:
                            cursor.execute("UPDATE StudentClassNumbers SET class_id=?, class_number=? WHERE id=?", (new_class_id, next_num, exists[0]))
                        else:
                            cursor.execute("INSERT INTO StudentClassNumbers (student_id, class_id, year_id, class_number) VALUES (?, ?, ?, ?)", (std_id, new_class_id, target_year_id, next_num))
                        
                        # ضمان أن حالة الطالب نشطة (في حال تم فصله سابقاً وأعيد قيده)
                        cursor.execute("UPDATE Students SET status='Active' WHERE id=?", (std_id,))
                    
                    count += 1
                
                # تفعيل السنة الجديدة أوتوماتيكياً إذا رغب المستخدم
