...
class IDCardPDF(FPDF):
    ...
    def generate_senegal_id_card(self, student):
        ...
        lines = [
            ...
            ("N° d'inscription :", str(student.get('class_number') or student['id'])),
...
class AdminDocsWindow(QMainWindow):
    ...
    def get_student_data(self, sid):
        ...
        try:
            ...
            with db.get_connection() as conn:
                cursor = conn.cursor()
                # تم التعديل لجلب address, parent_phone و registration_date لتغذية البطاقة
                cursor.execute("""
            ...
            if res:
                return {
                    'id': res[0], 'name': res[1], 'dob': res[2] if res[2] else "",
