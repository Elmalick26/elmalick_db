...
class StudentGradesWindow(QMainWindow):
    ...
    def load_periods(self, combo):
        try:
            ...
            with DatabaseManager() as db:
                conn = db.get_connection()
                cursor = conn.cursor()
                cursor.execute("SELECT id, period_name_fr FROM AcademicPeriods WHERE year_id=? ORDER BY sort_order", (active_year,))
                seen = set()
        ...
        try:
            ...
            with DatabaseManager() as db:
                ...
                if class_id:
                    ...
                    if row:
                        cursor.execute(
                            "SELECT id, period_name_fr FROM AcademicPeriods WHERE cycle_id=? AND year_id=? ORDER BY sort_order",
                            (row[0], active_year),
                        )
                    else:
                        cursor.execute(
                            "SELECT id, period_name_fr FROM AcademicPeriods WHERE year_id=? ORDER BY sort_order",
                            (active_year,),
                        )
                else:
                    cursor.execute(
    ...
    def on_class_changed_entry(self):
        ...
        try:
            with DatabaseManager() as db:
                ...
                cursor.execute("SELECT id, period_name_fr FROM AcademicPeriods WHERE cycle_id=? AND year_id=? ORDER BY sort_order", (cycle_id, active_year))
                for p in cursor.fetchall():
                    self.combo_period.addItem(p[1], p[0])
        except Exception as e:
            print(f"Error class changed: {e}")
    ...
    # ===== تعديل مهم: جلب الطلاب باستخدام جدول التسجيل SCN =====
    def load_grading_sheet(self):
        ...
        try:
            with DatabaseManager() as db:
                ...
                # تم التعديل: قراءة الطلاب من StudentClassNumbers واسترجاع الدرجات إن وجدت
                query = """
        ...
        try:
            with DatabaseManager() as db:
                ...
                for row in range(self.table_grades.rowCount()):
                    ...
                    cursor.execute("SELECT id FROM Grades WHERE student_id=? AND subject_id=? AND assessment_id=? AND year_id=?", (sid, subject_id, assess_id, active_year))
    ...
    # ===== تعديل مهم: البحث في درجات الطلاب باستخدام SCN =====
    def search_grades(self):
        ...
        try:
            with DatabaseManager() as db:
                ...
                # تم التعديل: جلب الفصل الخاص بالطالب من SCN بناءً على سنة تسجيل الدرجة
                query = """
    ...
    def print_sheet(self):
        ...
        try:
            with DatabaseManager() as db:
                ...
                # تم التعديل: جلب الطلاب للطباعة من SCN
                cursor.execute("""
                    SELECT S.id, S.first_name_fr || ' ' || S.last_name_fr,
                           S.first_name_ar || ' ' || S.last_name_ar
                    FROM Students S
