...
class AcademicSettingsWindow(QMainWindow):
    def __init__(self):
        ...
        header_frame = QFrame()
        if THEME_AVAILABLE:
            header_frame.setStyleSheet(f"background-color: {ThemeManager.get_colors().BG_HEADER}; border-radius: 10px;")
        else:
        ...
        if THEME_AVAILABLE:
            header_label.setStyleSheet(f"color: {ThemeManager.get_colors().HEADER_TEXT}; background: transparent;")
        else:
    ...
    def setup_general_tab(self):
        ...
        # School Info Card
        card, card_layout = self.create_card("🏫 Informations de l'École / معلومات المدرسة")
        
        grid = QGridLayout()
        grid.setSpacing(15)
        
        # Define fields
        self.txt_school_name = self.styled_input("Nom de l'établissement")
        self.txt_school_location = self.styled_input("Adresse / Lieu")
        
        # Set current values
        self.txt_school_name.setText(self.config.school_name)
        self.txt_school_location.setText(self.config.school_location)

        grid.addWidget(QLabel("Nom de l'établissement / اسم المؤسسة:"), 0, 0)
        grid.addWidget(self.txt_school_name, 0, 1)
        
        grid.addWidget(QLabel("Adresse / العنوان:"), 1, 0)
        grid.addWidget(self.txt_school_location, 1, 1)
        
        btn_save = QPushButton("Enregistrer / حفظ")
        btn_save.setCursor(Qt.CursorShape.PointingHandCursor)
        btn_save.setStyleSheet(f"""
            QPushButton {{ background-color: {ThemeManager.get_colors().SUCCESS if THEME_AVAILABLE else Colors().SUCCESS}; color: white; border: none; padding: 10px 20px; border-radius: 6px; font-weight: bold; }}
            QPushButton:hover {{ background-color: {ThemeManager.get_colors().SUCCESS_HOVER if THEME_AVAILABLE else Colors().SUCCESS_HOVER}; }}
        """)
        btn_save.clicked.connect(self.save_general_settings)
        
        card_layout.addLayout(grid)
        card_layout.addWidget(btn_save, alignment=Qt.AlignmentFlag.AlignRight)
        
        layout.addWidget(card)
        layout.addStretch()
        
        self.tabs.addTab(tab, "  🏛️ Général / عام  ")

    def save_general_settings(self):
        """Save general settings to config"""
        name = self.txt_school_name.text()
        location = self.txt_school_location.text()
        
        self.config.set('APPLICATION', 'school_name', name)
        self.config.set('APPLICATION', 'school_location', location)
    ...
    def create_card(self, title):
        ...
        if THEME_AVAILABLE:
            lbl.setStyleSheet(f"color: {ThemeManager.get_colors().TEXT_PRIMARY}; margin-bottom: 10px;")
        else:
    ...
    def add_action_buttons(self, table, row_idx, id_val, edit_func=None, delete_func=None):
        ...
        if edit_func:
            ...
            if THEME_AVAILABLE:
                btn_edit.setStyleSheet(f"background-color: {ThemeManager.get_colors().WARNING}; color: white; border-radius: 4px; border: none;")
            btn_edit.clicked.connect(lambda: edit_func(id_val))
        ...
        if delete_func:
            ...
            if THEME_AVAILABLE:
                btn_del.setStyleSheet(f"background-color: {ThemeManager.get_colors().DANGER}; color: white; border-radius: 4px; border: none;")
            btn_del.clicked.connect(lambda: delete_func(id_val))
    ...
    def setup_structure_tab(self):
        ...
        self.table_years = QTableWidget(0, 4) # ID, Name, Active, Action
        ...
        self.table_cycles = QTableWidget(0, 4) # ID, FR, AR, Action
        ...
        self.sp_order.setStyleSheet(f"border: 1px solid {colors.BORDER}; border-radius: 6px; padding: 5px;")
        ...
        self.table_classes = QTableWidget(0, 5) # ID, Cycle, FR, AR, Action
    ...
    def setup_subjects_tab(self):
        ...
        self.sp_coeff.setStyleSheet(f"border: 1px solid {colors.BORDER}; border-radius: 6px; padding: 5px;")
    ...
    def setup_evaluations_tab(self):
        ...
        # Table with Arabic name column for Assessments
    ...
    def activate_year(self, id):
        ...
        db = DatabaseManager()
        with db.get_connection() as conn:
            # إزالة is_active من جميع السنوات
            conn.execute("UPDATE AcademicYears SET is_active=0")
            # تنشيط السنة المختارة
            conn.execute("UPDATE AcademicYears SET is_active=1 WHERE id=?", (id,))
            conn.commit()
        self.refresh_all_data()
        QMessageBox.information(self, "نجح", "تم تنشيط هذه السنة الدراسية!")

    def delete_year(self, id):
        if QMessageBox.question(self, "Supprimer", "Supprimer cette année ?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Yes:
            db = DatabaseManager()
            with db.get_connection() as conn:
    ...
    # 2. Cycles
    def save_cycle(self):
        fr, ar = self.txt_c_fr.text(), self.txt_c_ar.text()
        if not fr: return
        db = DatabaseManager()
        with db.get_connection() as conn:
    ...
    def save_class(self):
        ...
        fr, ar = self.txt_cls_fr.text(), self.txt_cls_ar.text()
    ...
    def save_subject(self):
        ...
        fr, ar = self.txt_sub_fr.text(), self.txt_sub_ar.text()
    ...
    # 5. Evaluations (Delete Only mostly, as manual edit is complex without form)
    ...
    def save_school_info(self):
        vals = [v.text() for k, v in self.info_inputs.items()] 
    ...
    def generate_academic_logic(self):
        ...
        if not year_id or not cycle_id: return
        ...
        self.refresh_all_data()
        QMessageBox.information(self, "Succès", "Configuration générée.")
    ...
    def refresh_all_data(self):
        ...
        with db.get_connection() as conn:
            ...
            if info:
                if len(info) > 1: self.info_inputs['rep'].setText(str(info[1]))
                if len(info) > 2: self.info_inputs['ia'].setText(str(info[2]))
                if len(info) > 3: self.info_inputs['ief'].setText(str(info[3]))
                if len(info) > 4: self.info_inputs['name'].setText(str(info[4]))
                if len(info) > 5: self.info_inputs['auth'].setText(str(info[5]))
                if len(info) > 6: self.info_inputs['loc'].setText(str(info[6]))
                if len(info) > 7: self.info_inputs['tel'].setText(str(info[7]))
                if len(info) > 8: 
                    self.logo_path_value = info[8]
                    self.logo_path_display.setText(os.path.basename(info[8]) if info[8] else "")
                if len(info) > 9: self.info_inputs['director'].setText(str(info[9]))
            else:
                # Fallback to Config for Name and Location if DB is empty
            ...
            for r in cursor.fetchall():
                ...
                self.table_years.setItem(idx, 1, QTableWidgetItem(r[1]))
                # عرض حالة التنشيط
                status_item = QTableWidgetItem("✓ نعم" if r[2] else "✕ لا")
                ...
                btn_activate = QPushButton("✓ تنشيط" if not r[2] else "✓ مفعل")
                ...
                self.combo_year_gen.addItem(r[1], r[0])
            ...
            for r in cursor.fetchall():
                ...
                self.table_cycles.setItem(idx, 1, QTableWidgetItem(r[1]))
                self.table_cycles.setItem(idx, 2, QTableWidgetItem(r[2]))
                self.add_action_buttons(self.table_cycles, idx, r[0], self.edit_cycle, self.delete_cycle)
                self.combo_cycles_cls.addItem(r[1], r[0])
                self.combo_cycles_sub.addItem(r[1], r[0])
                self.combo_cycle_gen.addItem(r[1], r[0])
            ...
            for r in cursor.fetchall():
                ...
                self.table_classes.setItem(idx, 1, QTableWidgetItem(r[1]))
                self.table_classes.setItem(idx, 2, QTableWidgetItem(r[2]))
                self.table_classes.setItem(idx, 3, QTableWidgetItem(r[3]))
            ...
            for r in cursor.fetchall():
                ...
                for c, v in enumerate(r): self.table_subjects.setItem(idx, c, QTableWidgetItem(str(v)))
            ...
            for r in cursor.fetchall():
                ...
                # Columns: ID, Cycle, Period, FR, AR, Weight, Action
                self.table_evals.setItem(idx, 0, QTableWidgetItem(str(r[0])))
                self.table_evals.setItem(idx, 1, QTableWidgetItem(r[1]))
                self.table_evals.setItem(idx, 2, QTableWidgetItem(r[2]))
                self.table_evals.setItem(idx, 3, QTableWidgetItem(r[3]))
                self.table_evals.setItem(idx, 4, QTableWidgetItem(r[4]))
                self.table_evals.setItem(idx, 5, QTableWidgetItem(str(r[6])))
                # For evaluations, we only add Delete for now as edit is complex without form
